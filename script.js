// ============================================================================
// --- IRONCORE MAIN ENGINE (PRE-ALFA TESTNET - 100% BLINDADO Y PULIDO) ---
// ============================================================================
...
const customCSS = `
  ...
  #user-welcome-box img#user-avatar { width: 40px !important; height: 40px !important; border-radius: 50%; border: 2px solid #ffaa00; background:#1a2130; object-fit:cover; }
  ...
  .social-post-card { background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; margin-bottom: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.3); position:relative; }
  .social-post-header { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
  .social-post-avatar { width: 35px; height: 35px; border-radius: 50%; border: 1px solid #00e5ff; background:#1a2130;}
  .social-post-body { font-size: 13px; color: #e5e7eb; line-height: 1.5; margin-bottom: 10px; }
  .energy-ring { width: 160px; height: 160px; border-radius: 50%; background: conic-gradient(var(--primary) var(--pct), #1a2130 0deg); display: flex; align-items: center; justify-content: center; margin: 0 auto 20px auto; position: relative; box-shadow: 0 0 20px rgba(0,229,255,0.2); }
  .energy-ring::before { content: ""; position: absolute; width: 140px; height: 140px; background: #111827; border-radius: 50%; box-shadow: inset 0 4px 10px rgba(0,0,0,0.5); }
  ...
  .stat-panel { background: #1a2130; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; display: flex; flex-direction: column; justify-content: center; }
`;
...
window.moverOnboarding = function(dir) { 
  ...
  if(dir === 1 && currentObStep === 0) { const nickVal = document.getElementById('ob-nickname')?.value.trim(); if(!nickVal) { window.showToast('⚠️ Elige un apodo.'); return; } } 
...
window.cargarDatosDesdeNube = async function(uid) { 
  ...
  if (docSnap.exists()) { const d = docSnap.data(); totalCalorias = d.calorias || 0; totalProt = d.proteina || 0; totalCarb = d.carbos || 0; totalGrasa = d.grasa || 0; totalAgua = d.agua || 0; totalQuemadas = d.quemadas || 0; metaCalorias = d.metaCal || 2500; metaProt = d.metaProt || 165; metaCarb = d.metaCarb || 275; metaGrasa = d.metaGrasa || 69; userStreak = d.streak || 1; userProfile.perfilCompleto = d.perfilCompleto || false; userProfile.nickname = d.nickname || ""; userProfile.genero = d.genero || "M"; userProfile.edad = d.edad || 25; userProfile.peso = d.peso || 75; userProfile.altura = d.altura || 175; userProfile.metaObj = d.metaObj !== undefined ? d.metaObj : 0; userProfile.actividad = d.actividad || 1.55; currentRankName = d.currentRankName || "Ashigaru"; userProfile.avatarStyle = d.avatarStyle || "bottts"; 
    // Parche automático para cuentas viejas sin nickname_lower
...
window.actualizarUIPerfil = function() { 
  ...
  let profUI = document.getElementById('iron-profile-ui-advanced');
  if(!profUI) { profUI = document.createElement('div'); profUI.id = 'iron-profile-ui-advanced'; profPage.insertBefore(profUI, profPage.firstChild); }
  ...
  profUI.innerHTML = `
    <h2 class="page-title">Expediente de Combate</h2>
    <div class="card card-main" style="text-align: center; position: relative; background: #111827; border: 1px solid rgba(0,229,255,0.2);">
      <div style="position:relative; display:inline-block;">
        <img src="${avatarUrl}" style="width: 90px; height: 90px; border-radius: 50%; border: 3px solid var(--primary); margin-bottom: 10px; object-fit: cover; background: #1a2130; box-shadow: 0 0 15px rgba(0,229,255,0.3);">
        <button onclick="window.cambiarEstiloAvatar()" style="position:absolute; bottom:10px; right:-10px; background:var(--primary); color:#000; border:none; border-radius:50%; width:30px; height:30px; font-size:16px; cursor:pointer; box-shadow:0 2px 5px rgba(0,0,0,0.5);">🎲</button>
      </div>
      <h2 style="margin: 0; color: #fff; font-size: 24px; text-transform: uppercase; font-weight:900;">${userProfile.nickname}</h2>
      <div style="color:var(--accent); font-size:12px; font-weight:bold; margin-top:5px; text-transform:uppercase;">${currentRankName}</div>
      <span style="display:inline-block; padding: 4px 10px; background: rgba(0,229,255,0.1); color: var(--primary); font-size: 10px; font-weight:800; border-radius: 12px; margin-top: 10px; text-transform: uppercase;">Meta: ${labelMeta}</span>
      
      <div class="stats-grid" style="margin-top: 25px; text-align: left;">
        <div class="stat-panel"><div class="stat-title">Peso Corporal</div><div class="stat-value" style="font-size:18px;">${userProfile.peso} kg</div></div>
        <div class="stat-panel"><div class="stat-title">Altura</div><div class="stat-value" style="font-size:18px;">${userProfile.altura} cm</div></div>
        <div class="stat-panel"><div class="stat-title">Edad</div><div class="stat-value" style="font-size:18px;">${userProfile.edad} años</div></div>
      ...
      <button class="iron-btn-primary" style="margin-top:20px;" onclick="window.abrirOnboarding(true)">⚙️ Editar Biometría</button>
      <button class="iron-btn-danger" style="margin-top: 10px;" onclick="window.cerrarSesionApp()">🚪 Cerrar Sesión</button>
    </div>
  `;
};
...
window.dibujarGraficoPerfil = function(historialPesos) {
  ...
  canvasWrapper.innerHTML = `<div style="background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; margin-top: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);"><h4 style="font-size: 12px; color: #9ca3af; text-transform: uppercase; font-weight: 800; letter-spacing: 1px; margin-bottom: 15px;">📊 Proyección y Evolución</h4><div style="height: 250px; width: 100%;"><canvas id="profile-chart-canvas-inj"></canvas></div></div>`;
  ...
  if(realData.length > 0) {
    ...
    let dailyChange = userProfile.metaObj === -500 ? -0.07 : userProfile.metaObj === -300 ? -0.04 : userProfile.metaObj === 300 ? 0.04 : 0;
    projectionData = labels.map((_, i) => parseFloat((startWeight + (dailyChange * i * 7)).toFixed(1))); // Assuming approx weekly checkins
  }
  ...
  profileChartInstance = new Chart(ctx.getContext('2d'), {
    ...
    data: {
      labels: labels,
      datasets: [
        { label: 'Peso Real', data: realData, borderColor: '#00e5ff', backgroundColor: 'rgba(0, 229, 255, 0.1)', borderWidth: 3, fill: true, tension: 0.4, pointBackgroundColor: '#ffaa00', pointRadius: 5 },
        { label: 'Proyección Ideal', data: projectionData, borderColor: 'rgba(255, 170, 0, 0.6)', borderDash: [5, 5], borderWidth: 2, fill: false, pointRadius: 0 }
      ]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, labels: { color: '#fff' } } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks:{color:'#9ca3af'} }, x: { grid: { display: false }, ticks:{color:'#9ca3af'} } } }
  });
};
...
window.actualizarDashboard = function() { 
  ...
  dashUI.innerHTML = `
    <button id="btn-checkin-new" class="iron-btn-warning" style="margin-bottom: 20px; font-size:12px;" onclick="window.openSheet('sheet-checkin')">📈 Registrar Check-in Diario</button>
    <div class="card" style="background:#111827; border:1px solid rgba(255,255,255,0.05); padding:20px;">
      <div style="display:flex; justify-content:space-between; margin-bottom:20px;">
        <h3 style="margin:0; font-size:14px; color:#fff; text-transform:uppercase;">Monitor de Energía</h3>
        <button class="btn-clear-history" onclick="window.reiniciarDiaActual()">🔄 Reiniciar</button>
      </div>
      
      <div class="energy-ring" style="--pct: ${calPct}%; --primary: ${calColor};">
        <div class="energy-ring-content">
          <span style="font-size:32px; font-weight:900; color:${calColor}; text-shadow:0 0 10px rgba(0,229,255,0.3); display:block; line-height:1;">${calNetas}</span>
          <span style="font-size:10px; color:#9ca3af; text-transform:uppercase; font-weight:800;">KCAL NETAS</span>
        </div>
      </div>

      <div style="display:flex; justify-content:space-around; text-align:center; margin-bottom:20px; border-top:1px dashed rgba(255,255,255,0.1); padding-top:15px;">
        <div><span style="font-size:10px; color:#9ca3af; font-weight:800; display:block;">CONSUMIDAS</span><b style="color:#00e5ff; font-size:14px;">+${totalCalorias}</b></div>
        <div><span style="font-size:10px; color:#9ca3af; font-weight:800; display:block;">QUEMADAS</span><b style="color:#ffaa00; font-size:14px;">🔥-${totalQuemadas}</b></div>
        <div><span style="font-size:10px; color:#9ca3af; font-weight:800; display:block;">META DÍA</span><b style="color:#fff; font-size:14px;">${metaCalorias}</b></div>
      </div>

      <div class="stats-grid" style="margin-top:0;">
        <div class="stat-panel"><div class="stat-title">Proteína</div><div class="stat-value" style="font-size:16px;">${totalProt}g <span style="font-size:10px; color:#9ca3af;">/ ${metaProt}g</span></div><div class="macro-progress-container"><div class="macro-progress-fill" style="width:${prPct}%; background:#ff3366;"></div></div></div>
        <div class="stat-panel"><div class="stat-title">Carbohidratos</div><div class="stat-value" style="font-size:16px;">${totalCarb}g <span style="font-size:10px; color:#9ca3af;">/ ${metaCarb}g</span></div><div class="macro-progress-container"><div class="macro-progress-fill" style="width:${cbPct}%; background:#00e5ff;"></div></div></div>
        <div class="stat-panel stat-panel-full"><div class="stat-title">Grasas</div><div class="stat-value" style="font-size:16px;">${totalGrasa}g <span style="font-size:10px; color:#9ca3af;">/ ${metaGrasa}g</span></div><div class="macro-progress-container"><div class="macro-progress-fill" style="width:${grPct}%; background:#ffaa00;"></div></div></div>
      </div>
    </div>
    
    <div class="card" style="background:#111827; border:1px solid rgba(255,255,255,0.05); padding:20px; margin-top:20px;">
      <div style="display:flex; justify-content:space-between; margin-bottom:15px;"><h3 style="margin:0; font-size:14px; color:#fff; text-transform:uppercase;">💧 Hidratación</h3><button style="background:none; border:none; color:var(--primary); font-size:12px; font-weight:bold; cursor:pointer;" onclick="window.resetAgua()">Reset</button></div>
      <div style="display:flex; align-items:center; gap:15px;">
        <div style="flex:1;"><div class="stat-value" style="font-size:18px; color:#00e5ff;">${totalAgua/1000}L <span style="font-size:10px; color:#9ca3af;">/ 3.0L</span></div><div class="macro-progress-container"><div class="macro-progress-fill" id="water-fill-bar" style="width:${Math.min(100, (totalAgua/3000)*100)}%; background:#00e5ff;"></div></div></div>
        <button class="iron-btn-primary" style="width:auto; margin:0; padding:8px 12px; font-size:12px;" onclick="window.agregarAgua(500)">+ 500ml</button>
      </div>
    </div>
  `;
};
...
// --- ESCÁNER Y BUSCADOR MANUAL DE COMIDAS ---
...
window.onScanSuccess = async function(decodedText) {
  ...
  try {
    ...
    } else { window.showToast('❌ No encontrado.'); }
  } catch (err) { window.showToast('⚠️ Error conexión.'); }
};
...
window.borrarPostSocial = async function(postId) { if(!confirm('¿Borrar publicación?')) return; await deleteDoc(doc(db, "social_posts", postId)); window.showToast('🗑️ Eliminada'); window.cargarMuroSocialPrivado(); };
...
document.getElementById('btn-start-workout')?.addEventListener('click', () => { window.renderizarRutina(true); document.getElementById('btn-start-workout').style.display = 'none'; document.getElementById('btn-finish-workout').style.display = 'block'; window.showToast('🔥 ¡En vivo!'); });
...
window.eliminarComidaNube = async function(docId, cal, prot, carb, gras, btnElement) { if(!confirm("¿Eliminar?")) return; totalCalorias = Math.max(0, totalCalorias - cal); totalProt = Math.max(0, totalProt - prot); totalCarb = Math.max(0, totalCarb - carb); totalGrasa = Math.max(0, totalGrasa - gras); btnElement.closest('li')?.remove(); window.actualizarDashboard(); window.guardarEstadoNube(); if(currentUser && db && docId) await deleteDoc(doc(db, "users", currentUser.uid, "days", window.getTodayKey(), "meals", docId)); };
window.eliminarEntrenoNube = async function(docId, cals, btnElement) { if(!confirm("¿Eliminar?")) return; totalQuemadas = Math.max(0, totalQuemadas - cals); btnElement.closest('li')?.remove(); window.actualizarDashboard(); window.guardarEstadoNube(); if(currentUser && db && docId) await deleteDoc(doc(db, "users", currentUser.uid, "days", window.getTodayKey(), "workouts", docId)); };

window.registrarComidaNube = async function(cal, prot, carb, gras, nombreDisplay) { totalCalorias += cal; totalProt += prot; totalCarb += carb; totalGrasa += gras; await window.guardarEstadoNube(); window.actualizarDashboard(); let docId = null; if(currentUser && db) { const d = await addDoc(collection(db, "users", currentUser.uid, "days", window.getTodayKey(), "meals"), { nombre: nombreDisplay, cal, prot, carb, gras, timestamp: Date.now() }); docId = d.id; await addDoc(collection(db, "users", currentUser.uid, "history"), { tipo: 'comida', nombre: nombreDisplay, detalle: `🔥 ${cal} kcal | P:${prot}g C:${carb}g G:${gras}g`, date: window.getTodayKey(), timestamp: Date.now() }); } window.renderizarComidaEnUI(nombreDisplay, cal, prot, carb, gras, docId); if(currentUser) window.cargarHistorialYCheckins(currentUser.uid); document.querySelector('[data-target="page-dashboard"]')?.click(); };
window.registrarEntrenoNube = async function(nombre, sets, weight, rpe, cals = 0) { totalQuemadas += cals; await window.guardarEstadoNube(); window.actualizarDashboard(); let docId = null; if(currentUser && db) { const d = await addDoc(collection(db, "users", currentUser.uid, "days", window.getTodayKey(), "workouts"), { nombre, sets, weight, rpe, cals, timestamp: Date.now() }); docId = d.id; await addDoc(collection(db, "users", currentUser.uid, "history"), { tipo: 'entreno', nombre: nombre.toUpperCase(), detalle: `${sets} | 🔥 ${cals} kcal`, date: window.getTodayKey(), timestamp: Date.now() }); } window.renderizarEntrenoEnUI(nombre, sets, weight, rpe, cals, docId); if(currentUser) window.cargarHistorialYCheckins(currentUser.uid); };
...
window.cargarHistorialYCheckins = async function(uid) {
  ...
  if(chkContainer && db) {
    chkContainer.innerHTML = '<p style="text-align:center; font-size:12px; color:#9ca3af;">Cargando check-ins...</p>'; const q = query(collection(db, "users", uid, "checkins"), orderBy("timestamp", "desc"), limit(15)); const snaps = await getDocs(q); let html = ''; let historialPesos = []; snaps.forEach(d => { let data = d.data(); historialPesos.push(data); html += `<div style="background:#111827; padding:12px; margin-bottom:10px; border-radius:8px; border-left:3px solid #ffaa00; display:flex; justify-content:space-between; align-items:center;"><span style="color:#00e5ff; font-weight:800; font-size:12px;">📅 ${data.fecha}</span><span style="color:#fff; font-weight:900; font-size:14px;">⚖️ ${data.peso} kg</span></div>`; }); chkContainer.innerHTML = html || '<p style="text-align:center; font-size:12px; color:#9ca3af;">No hay check-ins registrados.</p>'; if(historialPesos.length > 0) window.dibujarGraficoPerfil(historialPesos);
  }
  if(histContainer && db) {
    histContainer.innerHTML = '<p style="text-align:center; font-size:12px; color:#9ca3af;">Cargando historial...</p>'; const q2 = query(collection(db, "users", uid, "history"), orderBy("timestamp", "desc"), limit(60)); const snaps2 = await getDocs(q2); let agrupado = {}; snaps2.forEach(d => { let data = d.data(); if(!agrupado[data.date]) agrupado[data.date] = []; agrupado[data.date].push(data); }); let html2 = ''; for(const [fecha, items] of Object.entries(agrupado)) { let itemsHtml = items.map(i => `<div style="margin-top:10px; padding-bottom:10px; border-bottom:1px solid rgba(255,255,255,0.05);"><b style="color:${i.tipo === 'entreno' ? '#00e5ff' : '#ff3366'}; font-size:13px;">${i.tipo === 'entreno' ? '🏋️' : '🍏'} ${i.nombre}</b><br><span style="font-size:11px; color:#9ca3af; display:block; margin-top:4px;">${i.detalle}</span></div>`).join(''); html2 += `<div class="history-day"><div class="history-day-header" onclick="const content = this.nextElementSibling; content.style.display = content.style.display === 'block' ? 'none' : 'block';"><span>📅 Jornada: ${fecha}</span> <span style="color:#fff;">▼</span></div><div class="history-day-content">${itemsHtml}</div></div>`; } histContainer.innerHTML = html2 || '<p style="text-align:center; font-size:12px; color:#9ca3af;">Aún no hay historial.</p>';
  }
};

window.iniciarSakuraBackground = function() { const c = document.getElementById('sakura-bg'); if(!c) return; c.innerHTML = ''; for(let i=0; i<15; i++) { const p = document.createElement('div'); p.className = 'sakura-petal'; const s = Math.random() * 8 + 4; p.style.width = `${s}px`; p.style.height = `${s*1.4}px`; p.style.left = `${Math.random()*100}vw`; p.style.animationDuration = `${Math.random()*10+8}s`; p.style.animationDelay = `${Math.random()*5}s`; c.appendChild(p); } };
...
document.getElementById('btn-calcular-rango')?.addEventListener('click', async () => { const bench = parseFloat(document.getElementById('rm-bench')?.value) || 0; const squat = parseFloat(document.getElementById('rm-squat')?.value) || 0; const deadlift = parseFloat(document.getElementById('rm-deadlift')?.value) || 0; if(bench === 0 && squat === 0 && deadlift === 0) { window.showToast('⚠️ Ingresa marcas.'); return; } const total = bench + squat + deadlift; const ratio = parseFloat((total / userProfile.peso).toFixed(2)); let rango = rangos[0], prog = 0; for(let i=0; i<rangos.length; i++) { if(ratio >= rangos[i].minRatio) { rango = rangos[i]; prog = i < rangos.length - 1 ? ((ratio - rangos[i].minRatio) / (rangos[i+1].minRatio - rangos[i].minRatio)) * 100 : 100; } } const rt = document.getElementById('rango-titulo'); if(rt) { rt.innerText = rango.nombre; rt.style.color = rango.color; } const rm = document.getElementById('rango-multi'); if(rm) rm.innerText = `${ratio}x`; const rtot = document.getElementById('rango-total'); if(rtot) rtot.innerText = total; const rp = document.getElementById('rango-progreso'); if(rp) { rp.style.width = `${prog}%`; rp.style.backgroundColor = rango.color; } const hr = document.getElementById('header-rank'); if(hr) hr.innerHTML = `<span style="color: ${rango.color};">${rango.nombre.toUpperCase()}</span>`; currentRankName = rango.nombre; window.actualizarUIHeader(); window.actualizarUIPerfil(); if(currentUser && db) { try { await setDoc(doc(db, "leaderboard", currentUser.uid), { userId: currentUser.uid, nombre: userProfile.nickname, foto: currentUser.photoURL || "", multiplicador: ratio, totalKg: total, pesoCorporal: userProfile.peso, rango: rango.nombre, colorRango: rango.color, updatedAt: Date.now() }, { merge: true }); window.showToast(`⚔️ ¡Ranking Actualizado!`); await window.guardarEstadoNube(); window.cargarLeaderboard(); } catch(e) {} } });
window.cargarLeaderboard = async function() { const container = document.getElementById('leaderboard-list'); if(!container || !db) return; container.innerHTML = `<p style="font-size: 12px; color: #9ca3af; text-align: center;">Cargando guerreros...</p>`; try { const q = query(collection(db, "leaderboard"), orderBy("multiplicador", "desc"), limit(20)); const snapshot = await getDocs(q); if(snapshot.empty) { container.innerHTML = `<p style="font-size: 12px; color: #9ca3af; text-align: center;">El dojo está vacío.</p>`; return; } let html = "", pos = 1; snapshot.forEach(docSnap => { const d = docSnap.data(); const topClass = pos===1?"top-1":pos===2?"top-2":pos===3?"top-3":""; const medal = pos===1?"🥇":pos===2?"🥈":pos===3?"🥉":`#${pos}`; html += `<div class="leaderboard-item ${topClass}"><div class="lb-rank-num">${medal}</div><div class="lb-user-info"><img class="lb-avatar" src="${window.generarAvatarPorRango(d.nombre, d.rango, 'bottts')}"><div><span class="lb-name">${d.nombre}</span><span class="lb-badge" style="color: ${d.colorRango};">${d.rango}</span></div></div><div class="lb-score"><span class="lb-multiplier">${d.multiplicador}x</span><span class="lb-kg">${d.totalKg} kg</span></div></div>`; pos++; }); container.innerHTML = html; } catch(e) { container.innerHTML = `<p style="font-size: 12px; color: #ff3366; text-align: center;">Error al cargar.</p>`; } };

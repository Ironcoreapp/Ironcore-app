...
const customCSS = `
  ...
  /* Advanced Dashboard Energy Ring */
...
// Modal Motivacional
const oraculoQuotes = ["La disciplina es el puente entre metas y logros.", "El hierro no miente.", "Cada repetición te acerca a tu mejor versión.", "La gravedad es resistencia, véncela.", "La constancia vence a la intensidad.", "El dolor de hoy es la victoria del mañana."];
...
window.cargarDatosDesdeNube = async function(uid) { 
  ...
  window.iniciarSakuraBackground(); window.verificarCambioDeDia(); window.actualizarDashboard(); window.actualizarAguaUI(); await window.cargarRegistrosDelDia(uid); await window.cargarHistorialYCheckins(uid); 
};
...
// --- MEJORA DEL PERFIL (AVATAR CUSTOM Y MÉTRICAS) ---
window.actualizarUIPerfil = function() { 
  ...
  if(!profUI) {
    profPage.innerHTML = '<h2 class="page-title">Expediente de Combate</h2><div id="iron-profile-ui-advanced"></div>';
    profUI = document.getElementById('iron-profile-ui-advanced');
  }
...
// --- MEJORA DEL DASHBOARD (ANILLO DE ENERGÍA Y GRID) ---
window.actualizarDashboard = function() { 
  ...
  if(!dashUI) {
    ...
    dashUI = document.createElement('div'); dashUI.id = 'iron-dash-ui-advanced';
    dashboardContainer.appendChild(dashUI);
  }
  ...
  dashUI.innerHTML = `
    ...
    <div class="card" style="background:#111827; border:1px solid rgba(255,255,255,0.05); padding:20px; margin-top:20px;">
      ...
      <div style="display:flex; align-items:center; gap:15px;">
        <div style="flex:1;"><div class="stat-value" style="font-size:18px; color:#00e5ff;">${totalAgua/1000}L <span style="font-size:10px; color:#9ca3af;">/ 3.0L</span></div><div class="macro-progress-container"><div class="macro-progress-fill" style="width:${Math.min(100, (totalAgua/3000)*100)}%; background:#00e5ff;"></div></div></div>
...
async function inicializarBases() {
  try { const resFood = await fetch('alimentos.json?v=' + Date.now()); if (resFood.ok) oracleDB = await resFood.json(); } catch (e) { console.error("Error Alimentos JSON"); }
  try { const resEx = await fetch('ejercicios.json?v=' + Date.now()); if (!resEx.ok) throw new Error(); exercisesDB = await resEx.json(); console.log(`⚔️ Base sincronizada con ${exercisesDB.length} ejercicios.`); } catch (e) { console.error("Error ejercicios.json"); }
}
inicializarBases();

// --- FUNCIONES NUTRICIÓN Y COMPRAS BLINDADAS EN WINDOW ---
...
document.getElementById('btn-trigger-search')?.addEventListener('click', async () => {
  ...
  } catch (error) { loader.style.display = 'none'; c.innerHTML = `<div style="font-size:12px; color:#ff3366; text-align:center; margin-top:10px;">❌ No encontrado en línea. Intenta escanear el código.</div>`; }
});
...
document.getElementById('btn-generar-rutina')?.addEventListener('click', () => {
  ...
  if(currentWorkoutRoutine.length === 0) { window.showToast('⚠️ Error.'); return; }
...
// 10. SOCIAL / COFRADÍA (CON BORRADO)
...
window.actualizarAguaUI = function() { const metaAgua = 3000; let pct = Math.min(100, Math.round((totalAgua/metaAgua)*100)); const wb = document.getElementById('water-fill-bar'); if(wb) wb.style.height = `${pct}%`; const wt = document.getElementById('water-text-val'); if(wt) wt.innerText = `${totalAgua} / ${metaAgua} ml`; window.guardarEstadoNube(); };
...
document.getElementById('btn-confirm-checkin')?.addEventListener('click', async function() { if (this.disabled) return; this.disabled = true; const pesoInp = document.getElementById('checkin-peso'); const pesoVal = pesoInp ? parseFloat(pesoInp.value) : 0; if(!pesoVal) { window.showToast('⚠️ Por favor ingresa tu peso.'); this.disabled = false; return; } if(currentUser && db) { await addDoc(collection(db, "users", currentUser.uid, "checkins"), { peso: pesoVal, fecha: window.getTodayKey(), timestamp: Date.now() }); userProfile.peso = pesoVal; await window.guardarEstadoNube(); window.actualizarUIPerfil(); window.cargarHistorialYCheckins(currentUser.uid); } window.closeSheet(); if(pesoInp) pesoInp.value = ''; window.showToast('📈 Check-in guardado.'); this.disabled = false; });

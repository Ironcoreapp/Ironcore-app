...
window.cambiarEstiloAvatar = async function() {
  ...
  window.actualizarUIHeader(); window.actualizarUIPerfil();
...
// --- CONSTRUCCIÓN DEL PERFIL DE ALTA GAMA ---
...
// --- GRÁFICO EVOLUCIÓN (PROYECCIÓN IDEAL MATEMÁTICA) ---
...
window.dibujarGraficoPerfil = function(historialPesos) {
  ...
  if(realData.length > 0) {
    ...
    // Déficit Agresivo: -0.7kg/semana. Definición: -0.3kg/semana. Volumen: +0.3kg/semana
    let dailyChange = userProfile.metaObj === -500 ? -0.1 : userProfile.metaObj === -300 ? -0.04 : userProfile.metaObj === 300 ? 0.04 : 0;
    projectionData = labels.map((_, i) => parseFloat((startWeight + (dailyChange * i * 3)).toFixed(1))); // Estimando 3 dias entre checkins promedio
  }
...
// --- DASHBOARD: ANILLO DE ENERGÍA DE ALTA GAMA ---
...
async function inicializarBases() {
  ...
  try { const resEx = await fetch('ejercicios.json?v=' + Date.now()); if (!resEx.ok) throw new Error(); exercisesDB = await resEx.json(); console.log(`⚔️ Base sincronizada con ${exercisesDB.length} ejercicios.`); } catch (e) {}
}
inicializarBases();

// --- ORÁCULO DE NUTRICIÓN Y COMPRAS ---
...
document.getElementById('btn-generar-plan')?.addEventListener('click', () => { 
  if (oracleDB.length === 0) { window.showToast("⚠️ Sincronizando alimentos... Espera un segundo."); return; } 
  window.calcularFechasSemana(); window.generarPlanSemanal(); 
  document.getElementById('oracle-form-card').style.display = 'none'; document.getElementById('plan-resultado').style.display = 'block'; window.showToast('🤖 Plan clínico generado con éxito.'); 
});
...
window.confirmarReemplazo = function(encodedData) { if(swapTargetIndex > -1) { const newMeal = JSON.parse(decodeURIComponent(encodedData)); weeklyPlan[selectedDayIndex][swapTargetIndex] = newMeal; window.renderizarDiaSeleccionado(); window.closeSheet(); window.showToast(`✅ Reemplazo aplicado.`); } };
...
window.registrarComidaPlaneada = async function(encodedData) {
  ...
  await window.registrarComidaNube(meal.cals, meal.prot, meal.carb, meal.gras, nombreFinal); window.closeSheet(); window.showToast(`✅ Menú registrado.`);
};

// --- ESCÁNER Y BÚSQUEDA DIRECTA (OPENFOODFACTS) ---
...
window.onScanSuccess = async function(decodedText) {
  if (html5QrCode && html5QrCode.isScanning) { await html5QrCode.stop(); } window.closeScanner(); window.showToast(`🔍 Escaneando ${decodedText}...`);
  try {
    const res = await fetch(`https://world.openfoodfacts.org/api/v0/product/${decodedText}.json`); const data = await res.json();
    if (data.status === 1) {
      const p = data.product; if(!p.nutriments || !p.nutriments['energy-kcal_100g']) { window.showToast('⚠️ El producto no tiene info calórica.'); return; }
      ...
      window.currentSearchFoodBase = { cal: cal100, prot: prot100, carb: carb100, gras: gras100 }; document.getElementById('food-selected-name').innerText = nombre; document.getElementById('food-results-list').innerHTML = ''; document.getElementById('edit-qty').value = '100'; document.getElementById('edit-unit').value = '1'; window.recalcularMacros(); document.getElementById('food-custom-section').style.display = 'block'; window.openSheet('sheet-add-food');
    } else { window.showToast('❌ Producto no encontrado en la base global.'); }
...
document.getElementById('btn-trigger-search')?.addEventListener('click', async () => {
  ...
  try {
    // LLAMADA DIRECTA A OPENFOODFACTS SIN PROXY FALLIDO
    ...
    if (!res.ok) throw new Error('Fallo conexión'); 
    const data = await res.json(); loader.style.display = 'none'; if(!data.products || data.products.length === 0) throw new Error("Sin resultados"); 
    ...
    if(resultadosLocales.length > 0) { window.renderizarResultadosBusqueda(resultadosLocales, c); window.showToast('⚠️ Base local activada.'); } else { c.innerHTML = `<div style="font-size:12px; color:#ff3366; text-align:center; margin-top:10px;">❌ Cero coincidencias.</div>`; }
  }
});
...
document.getElementById('btn-confirm-food-final')?.addEventListener('click', async function() {
  ...
  let nombreFinalRegistro = `${n} (${qtyVal} ${unitText})`; await window.registrarComidaNube(c, p, cb, g, `[${mt}] ${nombreFinalRegistro}`); window.closeSheet(); window.showToast(`✅ Alimento guardado.`); this.disabled = false;
});

// --- SOCIAL / COFRADÍA (ENVIADAS, RECIBIDAS, BORRAR POSTS) ---
    ...
    try { 
      ...
      document.getElementById('btn-send-req').onclick = async () => { 
        try { 
          ...
          window.showToast("✅ Invitación en camino."); resBox.innerHTML = ''; window.cargarSolicitudesYAmigosSocial();
        } catch(e) { window.showToast("❌ Error de comunicación."); } 
      }; 
...
window.cargarMuroSocialPrivado = async function() {
  ...
  try {
    ...
    snaps.forEach(d => { let post = d.data(); if(allowedUids.includes(post.uid)) { const delBtn = post.uid === currentUser.uid ? `<button onclick="window.borrarPostSocial('${d.id}')" style="background:rgba(0,0,0,0.5); border:1px solid #ff3366; color:#ff3366; border-radius:6px; padding:3px 6px; cursor:pointer; font-size:10px; position:absolute; right:15px; top:15px; transition:0.2s;">🗑️</button>` : ''; html += `<div class="social-post-card">${delBtn}<div class="social-post-header"><img class="social-post-avatar" src="${window.generarAvatarPorRango(post.author, post.rango || 'Ashigaru', post.avatarStyle || 'bottts')}"><div><b style="color:#fff; font-size:14px; text-transform:uppercase; letter-spacing:1px;">${post.author}</b><br><span style="font-size:9px; color:#00e5ff; font-weight:800; letter-spacing:0.5px;">${post.rango || 'Guerrero'} • ${new Date(post.timestamp).toLocaleDateString()}</span></div></div><div class="social-post-body">${post.content}</div></div>`; } }); feedContainer.innerHTML = html || '<p style="font-size:12px; color:#9ca3af; text-align:center;">Muro silencioso. ¡Sé el primero en gritar victoria!</p>';
...
window.aceptarSolicitud = async function(reqDocId, fromUid, fromNickname) { 
  ...
  window.showToast(`⚔️ ¡Alianza forjada con ${fromNickname}!`); window.cargarSolicitudesYAmigosSocial(); window.cargarMuroSocialPrivado(); 
};
...
window.verPerfilAmigoCompleto = async function(friendUid) { 
  const docSnap = await getDoc(doc(db, "users", friendUid)); if(!docSnap.exists()) { window.showToast("⚠️ Extraviado en el combate."); return; } const data = docSnap.data(); 
  const workoutsSnap = await getDocs(query(collection(db, "users", friendUid, "history"), orderBy("timestamp", "desc"), limit(5))); let workoutsHtml = ''; workoutsSnap.forEach(w => { let item = w.data(); workoutsHtml += `<div style="font-size:11px; color:#00e5ff; margin-bottom:6px; background:rgba(0,0,0,0.3); padding:8px; border-radius:6px;">• <b style="color:#fff;">${item.nombre}</b><br><span style="color:var(--text-muted);">${item.detalle} (${item.date})</span></div>`; }); 
...
window.eliminarEntrenoNube = async function(docId, cals, nombre, btnElement) { 
  if(!confirm("¿Borrar misión táctica?")) return; 

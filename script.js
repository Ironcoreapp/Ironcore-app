// ============================================================================
// --- MOTOR PRINCIPAL IRONCORE (PRE-ALFA TESTNET - DEPURED & UNIFIED) ---
// ============================================================================
const customCSS = `
  ...
  document.querySelectorAll('.nav-item').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if(btn.getAttribute('data-target') === 'page-social') {
        if(typeof window.cargarModuloSocialCompleto === 'function') window.cargarModuloSocialCompleto();
      }
    });
  });
...
window.finalizarOnboarding = async function() {
  ...
  if (currentUser && db) {
    ...
    if (!snap.empty && snap.docs[0].id !== currentUser.uid) { showToast("⚠️ Ese apodo ya está en uso. Elige otro."); return; }
  }
  ...
  await guardarEstadoNube(); actualizarUIHeader(); actualizarUIPerfil(); window.actualizarDashboard(); 
...
async function cargarDatosDesdeNube(uid) { 
  ...
  iniciarSakuraBackground(); verificarCambioDeDia(); window.actualizarDashboard(); actualizarAguaUI(); await cargarRegistrosDelDia(uid); await cargarHistorialYCheckins(uid); 
}
...
// ⚠️ SOLO DEBE EXISTIR ESTA FUNCIÓN UNA VEZ ⚠️
...
if(allergyInput) { 
  allergyInput.addEventListener('keypress', (e) => { 
    if(e.key === 'Enter' || e.key === ',') { 
      e.preventDefault(); const val = allergyInput.value.trim().toLowerCase(); 
...
function generarPlanSemanal() { 
  weeklyPlan = []; 
  const distribution = [ { tipo: 'desayuno', cals: metaCalorias * 0.25 }, { tipo: 'almuerzo', cals: metaCalorias * 0.35 }, { tipo: 'cena', cals: metaCalorias * 0.30 }, { tipo: 'snack', cals: metaCalorias * 0.10 } ]; 
...
function calcularMacrosPorcion(comidaBase, targetCals) { 
  ...
  let ingredientesAdaptados = (comidaBase.ingredientes || []).map(ing => { 
    let qtyCalculada = ing.baseQty * factor; qtyCalculada = ing.unidad === 'unidades' || ing.unidad === 'scoops' ? parseFloat(qtyCalculada.toFixed(1)) : Math.round(qtyCalculada); 
  ...
  return { 
    ...
    cals: Math.round(comidaBase.calBase * factor), prot: Math.round(comidaBase.prot * factor), carb: Math.round(comidaBase.carb * factor), gras: Math.round(comidaBase.gras * factor), ingredientes: ingredientesAdaptados 
  }; 
}
...
function obtenerComidaAlgoritmo(tipo, targetCals, excludesId) { 
  ...
  const selected = candidatos[Math.floor(Math.random() * candidatos.length)]; return calcularMacrosPorcion(selected, targetCals); 
}
...
window.abrirMenuReemplazo = function(mealIndex) { 
  ...
  if(candidatos.length === 0) { listContainer.innerHTML = `<p style="font-size:12px; color:#ff3366;">No hay más opciones para tus filtros.</p>`; } else { 
    for(let i=0; i<candidatos.length; i++) { 
      let opcionEscalada = calcularMacrosPorcion(candidatos[i], targetCals); let objData = encodeURIComponent(JSON.stringify(opcionEscalada)); 
...
window.confirmarReemplazo = function(encodedData) { 
  if(swapTargetIndex > -1) { const newMeal = JSON.parse(decodeURIComponent(encodedData)); weeklyPlan[selectedDayIndex][swapTargetIndex] = newMeal; renderizarDiaSeleccionado(); window.closeSheet(); showToast(`✅ Actualizado.`); } 
};
...
window.registrarComidaPlaneada = async function(encodedData) {
  ...
  window.closeSheet(); showToast(`✅ Registrado al instante.`);
};
...
document.getElementById('btn-foto')?.addEventListener('click', () => { 
  ...
  html5QrCode.start({ facingMode: "environment" }, config, onScanSuccess, () => {}).catch(err => { console.error("Error cámara:", err); showToast("⚠️ No se pudo iniciar la cámara."); });
});
...
async function onScanSuccess(decodedText) {
  ...
  window.closeScanner(); showToast(`🔍 Buscando ${decodedText}...`);
  try {
    ...
    if (data.status === 1) {
      ...
      document.getElementById('food-selected-name').innerText = nombre; document.getElementById('food-results-list').innerHTML = ''; document.getElementById('edit-qty').value = '100'; document.getElementById('edit-unit').value = '1'; recalcularMacros(); document.getElementById('food-custom-section').style.display = 'block'; window.openSheet('sheet-add-food');
...
document.getElementById('btn-trigger-search')?.addEventListener('click', async () => {
  const query = document.getElementById('food-search').value.trim().toLowerCase(); const c = document.getElementById('food-results-list'); const loader = document.getElementById('food-loading');
  ...
  try {
    ...
    loader.style.display = 'none'; if(!data.products || data.products.length === 0) throw new Error("Sin resultados");
    renderizarResultadosBusqueda(data.products, c);
  } catch (error) {
    loader.style.display = 'none'; const resultadosLocales = fallbackDB.filter(p => p.product_name.toLowerCase().includes(query) || p.brands.toLowerCase().includes(query));
    if(resultadosLocales.length > 0) { renderizarResultadosBusqueda(resultadosLocales, c); showToast('⚠️ Mostrando base local.'); } else { c.innerHTML = `<div style="font-size:12px; color:#ff3366; text-align:center; margin-top:10px;">❌ No encontrado.</div>`; }
  }
});
...
function renderizarResultadosBusqueda(productos, contenedor) {
  productos.forEach(p => {
    ...
    d.addEventListener('click', () => { currentSearchFoodBase = { cal: cal100, prot: prot100, carb: carb100, gras: gras100 }; document.getElementById('food-selected-name').innerText = nombre; document.getElementById('food-results-list').innerHTML = ''; document.getElementById('edit-qty').value = '100'; document.getElementById('edit-unit').value = '1'; recalcularMacros(); document.getElementById('food-custom-section').style.display = 'block'; }); contenedor.appendChild(d);
  });
}
...
function recalcularMacros() {
  if(currentSearchFoodBase) {
    ...
    document.getElementById('edit-cal').value = Math.round(currentSearchFoodBase.cal * factor); document.getElementById('edit-prot').value = Math.round(currentSearchFoodBase.prot * factor); document.getElementById('edit-carb').value = Math.round(currentSearchFoodBase.carb * factor); document.getElementById('edit-gras').value = Math.round(currentSearchFoodBase.gras * factor);
  }
}
...
window.actualizarDashboard = function() { 
  ...
  if(!gridBox) {
    ...
    gridBox = document.createElement('div'); gridBox.id = 'iron-stats-grid'; gridBox.className = 'stats-grid'; dashboardContainer.insertBefore(gridBox, document.getElementById('lista-comidas')?.parentElement || dashboardContainer.children[1]);
  }
  const calNetas = Math.max(0, totalCalorias - totalQuemadas);
  const prPct = Math.min(100, (totalProt/metaProt)*100) || 0; const cbPct = Math.min(100, (totalCarb/metaCarb)*100) || 0; const grPct = Math.min(100, (totalGrasa/metaGrasa)*100) || 0; const calColor = calNetas > metaCalorias ? 'danger' : 'primary';
...
function dibujarGraficoPerfil(historialPesos) {
  const tabProfile = document.getElementById('page-profile'); if(!tabProfile) return;
  let canvasWrapper = document.getElementById('wrapper-profile-chart');
  if(!canvasWrapper) {
    canvasWrapper = document.createElement('div'); canvasWrapper.id = 'wrapper-profile-chart';
    canvasWrapper.innerHTML = `<div style="background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; margin-top: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);"><h4 style="font-size: 12px; color: #9ca3af; text-transform: uppercase; font-weight: 800; letter-spacing: 1px; margin-bottom: 15px;">📊 Evolución Corporal</h4><div style="height: 200px; width: 100%;"><canvas id="profile-chart-canvas"></canvas></div></div>`;
    tabProfile.appendChild(canvasWrapper);
  }
  const ctx = document.getElementById('profile-chart-canvas'); if(!ctx) return;
  if(profileChartInstance) profileChartInstance.destroy();
  const labels = historialPesos.map(item => item.fecha.split('-').slice(1).join('/')); const data = historialPesos.map(item => item.peso);
  profileChartInstance = new Chart(ctx.getContext('2d'), { type: 'line', data: { labels: labels.reverse(), datasets: [{ label: 'Peso (kg)', data: data.reverse(), borderColor: '#00e5ff', backgroundColor: 'rgba(0, 229, 255, 0.1)', borderWidth: 3, fill: true, tension: 0.4, pointBackgroundColor: '#ffaa00', pointRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' } }, x: { grid: { display: false } } } } });
}

// --- LOGS, REGISTROS Y CHECKINS ---
...
async function cargarHistorialYCheckins(uid) {
  ...
  if(histContainer && db) {
    ...
    let agrupado = {}; snaps2.forEach(d => { let data = d.data(); if(!agrupado[data.date]) agrupado[data.date] = []; agrupado[data.date].push(data); }); let html2 = '';
...
function renderizarEntrenoEnUI(nombre, sets, weight, rpe, cals = 0, docId = null) { const l = document.getElementById('lista-entrenos'); if(!l) return; const li = document.createElement('li'); if(docId) li.setAttribute('data-id', docId); li.innerHTML = `<span style="color:#fff; font-weight:800;">${nombre.toUpperCase()}</span><button class="btn-delete-item" onclick="window.eliminarEntrenoNube('${docId}', ${cals}, this)">🗑️</button><br><span style="color: #9ca3af; font-size: 11px; margin-top:5px; display:flex; gap:10px; align-items:center;"><span>🏋️ ${sets}</span><span>Peso Máx: ${weight} kg</span><span style="color:#ffaa00; font-weight:800;">🔥 ~${cals} kcal</span></span>`; l.appendChild(li); }

window.eliminarComidaNube = async function(docId, cal, prot, carb, gras, btnElement) { if(!confirm("¿Eliminar?")) return; totalCalorias = Math.max(0, totalCalorias - cal); totalProt = Math.max(0, totalProt - prot); totalCarb = Math.max(0, totalCarb - carb); totalGrasa = Math.max(0, totalGrasa - gras); btnElement.closest('li')?.remove(); guardarEstadoNube(); window.actualizarDashboard(); if(currentUser && db && docId) await deleteDoc(doc(db, "users", currentUser.uid, "days", getTodayKey(), "meals", docId)); };
window.eliminarEntrenoNube = async function(docId, cals, btnElement) { if(!confirm("¿Eliminar?")) return; totalQuemadas = Math.max(0, totalQuemadas - cals); btnElement.closest('li')?.remove(); guardarEstadoNube(); window.actualizarDashboard(); if(currentUser && db && docId) await deleteDoc(doc(db, "users", currentUser.uid, "days", getTodayKey(), "workouts", docId)); };

async function registrarComidaNube(cal, prot, carb, gras, nombreDisplay) { 
  totalCalorias += cal; totalProt += prot; totalCarb += carb; totalGrasa += gras; guardarEstadoNube(); window.actualizarDashboard(); let docId = null; 
...
async function registrarEntrenoNube(nombre, sets, weight, rpe, cals = 0) { 
  totalQuemadas += cals; guardarEstadoNube(); window.actualizarDashboard(); let docId = null; 
...
window.reiniciarDiaActual = function() { if(confirm("¿Reiniciar balance?")) { totalCalorias = 0; totalProt = 0; totalCarb = 0; totalGrasa = 0; totalAgua = 0; totalQuemadas = 0; guardarEstadoNube(); window.actualizarDashboard(); actualizarAguaUI(); const lc = document.getElementById('lista-comidas'); if(lc) lc.innerHTML = ''; showToast('🔄 Restablecido.'); } };
...
function setupTabs(btnClass, subTabClass) { 
  ...
  btns.forEach(btn => { 
    btn.addEventListener('click', () => { 
      ...
      btn.classList.add('active'); const targetTab = btn.getAttribute('data-tab'); const elem = document.getElementById(targetTab); 
      if(elem) elem.style.display = 'block'; if(targetTab === 'tab-leaderboard') window.cargarLeaderboard(); 
    }); 
  }); 
} 
...
document.getElementById('btn-confirm-checkin')?.addEventListener('click', async function() { 
  ...
  if(currentUser && db) { await addDoc(collection(db, "users", currentUser.uid, "checkins"), { peso: pesoVal, fecha: getTodayKey(), timestamp: Date.now() }); userProfile.peso = pesoVal; await guardarEstadoNube(); actualizarUIPerfil(); cargarHistorialYCheckins(currentUser.uid); }
...
window.cargarModuloSocialCompleto = function() {
  ...
  root.innerHTML = `
    <div style="background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
      ...
      <div style="display:flex; gap:8px; margin-bottom:12px;">
        <input type="text" id="input-search-friend" class="neon-input" placeholder="Apodo exacto del amigo..." style="font-size:12px; padding:8px 12px;">
        <button id="btn-search-friend" class="btn btn-primary" style="width:auto; margin-top:0; padding:8px 15px; font-size:12px;">Buscar</button>
      </div>
    ...
    <div style="background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px;">
      ...
      <div style="background: rgba(0,0,0,0.3); padding: 12px; border-radius: 8px; margin-bottom: 15px;">
        <textarea id="social-post-text" class="neon-input" placeholder="Comparte tu progreso solo visible para tu cofradía..." style="height: 60px; font-size:12px; resize:none;"></textarea>
        <button id="btn-publish-social" class="btn btn-primary" style="padding: 8px; font-size:12px; margin-top:8px;">📢 Publicar en el Muro</button>
      </div>
  ...
  document.getElementById('btn-search-friend')?.addEventListener('click', async () => {
    ...
    if(!val) return; resBox.innerHTML = '<p style="font-size:11px; color:#9ca3af;">Rastreando guerrero...</p>';
    try {
      const q = query(collection(db, "users"), where("nickname_lower", "==", val)); const snap = await getDocs(q);
      if(snap.empty) { resBox.innerHTML = '<p style="font-size:11px; color:#ff3366;">❌ No se encontró ningún guerrero.</p>'; return; }
      const friendDocId = snap.docs[0].id; const friendData = snap.docs[0].data();
      if(friendDocId === currentUser.uid) { resBox.innerHTML = '<p style="font-size:11px; color:#ffaa00;">⚠️ ¡Ese eres tú mismo!</p>'; return; }
      resBox.innerHTML = `
        <div style="background:rgba(0,229,255,0.05); border:1px solid rgba(0,229,255,0.2); padding:10px; border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
          <div><b style="color:#fff; font-size:13px;">${friendData.nickname}</b><br><span style="font-size:10px; color:#ffaa00;">Rango: ${friendData.currentRankName || 'Ashigaru'}</span></div>
          <button id="btn-send-req" class="btn btn-warning" style="width:auto; padding:5px 10px; font-size:11px;">+ Enviar Solicitud</button>
        </div>`;
      document.getElementById('btn-send-req').onclick = async () => {
        try { await addDoc(collection(db, "users", friendDocId, "friend_requests"), { fromUid: currentUser.uid, fromNickname: userProfile.nickname, status: "pendiente", timestamp: Date.now() }); showToast("✅ Solicitud enviada con éxito."); resBox.innerHTML = ''; } catch(e) { showToast("❌ Error al enviar solicitud."); }
      };
  ...
  document.getElementById('btn-publish-social')?.addEventListener('click', async () => {
    ...
    try { await addDoc(collection(db, "social_posts"), { uid: currentUser.uid, author: userProfile.nickname, rango: currentRankName, content: txt, timestamp: Date.now() }); document.getElementById('social-post-text').value = ''; showToast("📢 ¡Publicado en el muro!"); window.cargarMuroSocialPrivado(); } catch(e) { showToast("❌ Error al publicar."); }
  });
...
window.aceptarSolicitud = async function(reqDocId, fromUid, fromNickname) {
  ...
  await deleteDoc(doc(db, "users", currentUser.uid, "friend_requests", reqDocId)); showToast(`⚔️ ¡Ahora ${fromNickname} es parte de tu cofradía!`); window.cargarSolicitudesYAmigosSocial(); window.cargarMuroSocialPrivado();
};
...
window.eliminarAmigo = async function(friendDocId, friendUid) {
  ...
  const reciprocalQuery = query(collection(db, "users", friendUid, "friends"), where("friendUid", "==", currentUser.uid)); const recSnap = await getDocs(reciprocalQuery); recSnap.forEach(async (rDoc) => { await deleteDoc(rDoc.ref); });
  showToast("🗑️ Amistad eliminada."); window.cargarSolicitudesYAmigosSocial(); window.cargarMuroSocialPrivado();
};

window.verPerfilAmigoCompleto = async function(friendUid) {
  const docSnap = await getDoc(doc(db, "users", friendUid)); if(!docSnap.exists()) { showToast("⚠️ No se encontró al usuario."); return; }
  const data = docSnap.data(); const workoutsSnap = await getDocs(query(collection(db, "users", friendUid, "history"), orderBy("timestamp", "desc"), limit(5))); let workoutsHtml = '';
  workoutsSnap.forEach(w => { let item = w.data(); workoutsHtml += `<div style="font-size:11px; color:#00e5ff; margin-bottom:4px;">• ${item.nombre}: ${item.detalle} (${item.date})</div>`; });
  ...
  container.innerHTML = `
    <div style="padding:20px; text-align:center;">
      ...
      <div style="background:#111827; padding:12px; border-radius:8px; text-align:left; font-size:12px; color:#9ca3af; margin-bottom:15px;"><p><b>Peso Actual:</b> ${data.peso || '--'} kg</p><p><b>Altura:</b> ${data.altura || '--'} cm</p><p><b>Meta:</b> ${data.metaObj === 300 ? 'Volumen' : 'Definición / Mantenimiento'}</p></div>
      <div style="background:#1a2130; padding:12px; border-radius:8px; text-align:left; max-height:150px; overflow-y:auto; margin-bottom:15px;"><b style="color:#ffaa00; font-size:11px; text-transform:uppercase; display:block; margin-bottom:6px;">🏋️ Últimas Actividades</b>${workoutsHtml || '<span style="font-size:11px; color:#9ca3af;">Sin actividad reciente registrada.</span>'}</div>
      <button class="btn btn-primary" style="width:100%;" onclick="window.closeSheet()">Cerrar Expediente</button>
    </div>
...
function iniciarNotificacionesEnVivo(uid) {
  ...
  window.notifInterval = setInterval(async () => {
    ...
    try { const reqSnap = await getDocs(collection(db, "users", uid, "friend_requests")); let pendientes = 0; reqSnap.forEach(d => { if(d.data().status === 'pendiente') pendientes++; }); if(pendientes > (window.lastPendientesCount || 0) && window.lastPendientesCount !== undefined) { showToast("🔔 ¡Has recibido una nueva solicitud de amistad en la Cofradía!"); } window.lastPendientesCount = pendientes; } catch(e) {}
  }, 15000);
}
...
document.getElementById('btn-generar-rutina')?.addEventListener('click', () => {
  ...
  const getRandomEx = (grupoReq) => { const valid = exercisesDB.filter(ex => { const matchGrupo = ex.grupo?.toLowerCase() === grupoReq.toLowerCase(); const matchEquip = Array.isArray(ex.equipamiento) ? ex.equipamiento.some(eq => eq.toLowerCase().includes(equip.toLowerCase())) : true; return matchGrupo && matchEquip; }); if(valid.length === 0) { const fallbackGrupo = exercisesDB.filter(ex => ex.grupo?.toLowerCase() === grupoReq.toLowerCase()); return fallbackGrupo.length > 0 ? fallbackGrupo[Math.floor(Math.random() * fallbackGrupo.length)] : null; } return valid[Math.floor(Math.random() * valid.length)]; };
  let structure = []; if(focus === "fullbody") structure = ["piernas", "pecho", "espalda", "hombros", "core"]; if(focus === "superior") structure = ["pecho", "espalda", "hombros", "brazos", "core"]; if(focus === "piernas") structure = ["piernas", "piernas", "piernas", "core"];
...
window.abrirMenuReemplazoEj = function(index) {
  workoutSwapTargetIndex = index; const currentEx = currentWorkoutRoutine[index]; const alternativas = exercisesDB.filter(e => e.grupo === currentEx.grupo && e.id !== currentEx.id); const listContainer = document.getElementById('sheet-swap-ex-list'); if(!listContainer) return; listContainer.innerHTML = '';
  if(alternativas.length === 0) { listContainer.innerHTML = `<p style="font-size:12px; color:#ff3366;">No hay más alternativas de este grupo muscular.</p>`; } else { alternativas.forEach(alt => { let objData = encodeURIComponent(JSON.stringify(alt)); listContainer.innerHTML += `<div class="swap-option-card" onclick="window.confirmarReemplazoEj('${objData}')" style="background:#1a2130; padding:12px; border-radius:8px; cursor:pointer; margin-bottom:8px;"><span style="font-size:10px; color:#00e5ff; font-weight:800; text-transform:uppercase;">${alt.musculoPrincipal}</span><b style="display:block; color:#fff; margin-top:4px;">${alt.nombre}</b></div>`; }); } 
  window.openSheet('sheet-swap-exercise');
};

window.confirmarReemplazoEj = function(encodedData) {
  if(workoutSwapTargetIndex > -1) { const newEx = JSON.parse(decodeURIComponent(encodedData)); newEx.loggedSets = []; newEx.estimatedCals = Math.floor(Math.random() * 20) + 35; currentWorkoutRoutine[workoutSwapTargetIndex] = newEx; window.renderizarRutina(false); window.closeSheet(); showToast(`✅ Ejercicio reemplazado con éxito.`); }
};
...
window.registrarSerieIndividual = function(idx) {
  ...
  const lista = document.getElementById(`lista-series-${idx}`); lista.innerHTML += `<li style="margin-bottom:8px; padding-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.05); font-size:13px; color:#fff;">Serie ${ex.loggedSets.length}: <span style="color:#00e5ff; font-weight:900;">${reps} ${ex.isPhase?'min':`reps x ${peso} kg`}</span></li>`;
...
window.iniciarDescanso = function(idx, segundos) {
  ...
  const container = document.getElementById(`timer-container-${idx}`); const textSpan = document.getElementById(`time-left-${idx}`); container.style.display = 'block'; let timeLeft = segundos; textSpan.innerText = timeLeft;
...
document.getElementById('btn-start-workout')?.addEventListener('click', () => { window.renderizarRutina(true); document.getElementById('btn-start-workout').style.display = 'none'; document.getElementById('btn-finish-workout').style.display = 'block'; showToast('🔥 ¡Modo en vivo activado!'); });
...
document.getElementById('btn-finish-workout')?.addEventListener('click', async function() {
  if (this.disabled) return; this.disabled = true; let validExercises = 0; let sessionCals = 0;
  for(let i = 0; i < currentWorkoutRoutine.length; i++) {
    const ex = currentWorkoutRoutine[i];
    if(ex.loggedSets && ex.loggedSets.length > 0) {
      const totalSets = ex.loggedSets.length; const detalles = ex.loggedSets.map((s, idx) => `S${idx+1}: ${s.reps}${ex.isPhase?'min':`x${s.peso}kg`}`).join(' | '); const weightHighestOrLast = ex.loggedSets[ex.loggedSets.length - 1].peso; 
      let exCals = 0; if(ex.isPhase) { let totalMins = 0; ex.loggedSets.forEach(s => totalMins += s.reps); exCals = totalMins * 8; } else { ex.loggedSets.forEach(s => { exCals += 15 + (s.peso * s.reps * 0.03); }); } exCals = Math.round(exCals); sessionCals += exCals;
  ...
  if(validExercises > 0) { sessionCals = Math.round(sessionCals); totalQuemadas += sessionCals; await guardarEstadoNube(); window.actualizarDashboard(); showToast(`✅ Entrenamiento finalizado. 🔥 ~${sessionCals} kcal.`); } else { showToast('⚠️ Sin series registradas.'); }
  document.getElementById('workout-live-container').style.display = 'none'; document.getElementById('workout-generator-card').style.display = 'block'; document.getElementById('btn-start-workout').style.display = 'block'; document.getElementById('btn-finish-workout').style.display = 'none'; currentWorkoutRoutine = []; this.disabled = false; 
});
...
window.abrirBuscadorManual = function() {
  ...
  container.innerHTML = `
    <div style="padding: 20px;">
      ...
      <div style="position:relative;"><input type="text" id="search-manual-ex" class="neon-input" placeholder="🔍 Buscar por nombre o músculo..." autocomplete="off"><div id="search-manual-results" class="search-results-box" style="position:absolute; width:100%;"></div></div>
  ...
  searchInput.addEventListener('input', function() {
    ...
    if(matches.length > 0) {
      ...
      matches.slice(0, 10).forEach(match => {
        const div = document.createElement('div'); div.className = 'search-item'; div.innerHTML = `<span>${match.nombre}</span> <span class="search-item-muscle">${match.musculoPrincipal || 'General'}</span>`;
        div.onclick = () => { selectedExercise = match; manualSets = []; searchInput.value = match.nombre; resultsBox.style.display = 'none'; document.getElementById('manual-live-ui').style.display = 'block'; document.getElementById('manual-selected-name').innerText = match.nombre; document.getElementById('manual-sets-list').innerHTML = ''; document.getElementById('btn-save-manual-workout').style.display = 'none'; }; resultsBox.appendChild(div);
      });
  ...
  document.getElementById('btn-add-manual-set').addEventListener('click', () => {
    ...
    if(!r) return; manualSets.push({reps: r, peso: p});
    ...
    const d = parseInt(document.getElementById('manual-descanso').value) || 90; document.getElementById('manual-timer').style.display = 'block'; let timeLeft = d; document.getElementById('manual-time-left').innerText = timeLeft;
    if(window.manualInterval) clearInterval(window.manualInterval);
    window.manualInterval = setInterval(() => { timeLeft--; document.getElementById('manual-time-left').innerText = timeLeft; if(timeLeft <= 0) { clearInterval(window.manualInterval); document.getElementById('manual-timer').style.display='none'; showToast('⏰ ¡A la batalla!'); } }, 1000);
  });

  document.getElementById('btn-save-manual-workout').addEventListener('click', async function() {
    if(this.disabled) return; this.disabled = true; if(manualSets.length === 0) return;
    const detalles = manualSets.map((s, idx) => `S${idx+1}: ${s.reps}x${s.peso}kg`).join(' | '); const weightHighestOrLast = manualSets[manualSets.length - 1].peso;
    ...
    totalQuemadas += cals; await guardarEstadoNube(); window.actualizarDashboard();
    if(window.manualInterval) clearInterval(window.manualInterval); window.closeSheet(); showToast(`✅ Ejercicio Guardado. 🔥 ~${cals} kcal.`); this.disabled = false;
  });
};
